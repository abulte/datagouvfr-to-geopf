source: https://geoplateforme.github.io/sdk-entrepot/resolveurs/
...
A l'utilisation du SDK comme un exécutable, il y a 4 résolveurs d’instanciés :
  ...
  * `user` : un UserResolver
  * `store_entity` : un StoreEntityResolver
  * `datetime`: un DateResolver
  * `params`: un DictResolver avec les valeurs passées en `--params`
...
Il y a 4 résolveurs de base :
  ...
  * DictResolver: permet d'insérer les valeurs contenues dans un dictionnaire ;
  * FileResolver: insère les valeurs contenues dans un fichier ;
  * StoreEntityResolver: récupère des informations sur les entités depuis la GPF ;
  * UserResolver: récupère des informations de l'utilisateur courant depuis la GPF ;
  * DateResolver: insertion d'une date au format désiré.
...
Permet de résoudre des paramètres faisant référence à une entité sur la GPF. Utilisation du pattern `store_entity_regex` de la configuration.
...
Exemple (fonctionne après la livraison des données du [tutoriel 2 pour les flux vecteur](../tutoriel_2_flux_vecteur/) ) :
...
Utilisation, résolveur nommé _my_date_ et date actuelle `19/02/2024 14:01`:
