source: https://geoplateforme.github.io/sdk-entrepot/tutoriel_2_flux_vecteur/
...
Suivez la page [configuration](../configuration/) pour définir le fichier de configuration.
...
Les données que la Géoplateforme va traiter sont situées dans le dossier `test`. Le fichier `test.md5` permettra de valider les données téléversées côté Géoplateforme.
...
Ouvrez le fichier. Vous trouverez plus de détails dans la [documentation sur les workflows](../workflow/), mais vous pouvez dès à présent voir que le workflow est composé de 4 étapes. Il faudra lancer une commande pour chacune d'elles.
