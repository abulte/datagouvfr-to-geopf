source: https://geoplateforme.github.io/sdk-entrepot/comme-module/

# Utilisation comme module Python#

## Configuration#

Afin d'utiliser cette librairie comme module, vous devrez [écrire un fichier de configuration](../configuration/) comme pour les autres utilisations.
...
Pour livrer des données (dataset, annexe, static, metadata, key), vous pouvez utiliser les [fichiers descripteurs de livraison](../upload_descriptor/) et appeler la classe `Delivery`.
...
Pour livrer des dataset, vous pouvez utiliser les [fichiers descripteurs de livraison](../upload_descriptor/) et appeler la classe `UploadAction`. Cela sera plus simple d'un point de vue Python mais moins modulaire.
...
???+ note "Utiliser un datastore spécifique** Vous pouvez préciser l'id d'un autre datastore s'il ne faut pas utiliser celui indiqué en configuration :
...
On part ici du principe que vous avez déjà écrit [votre workflow](../workflow/).
